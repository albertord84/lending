...
class Welcome extends CI_Controller {
    ...
    public function afiliados() {
        if ($_SESSION['logged_role'] === 'AFFIL') {
            ...
            $_SESSION['affiliate_logged_transactions'] = $this->affiliate_model->load_transactions(
                    $_SESSION['affiliate_logged_datas']['code'], $datas['num_page'] - 1, $GLOBALS['sistem_config']->TRANSACTIONS_BY_PAGE, $datas['token'], NULL, NULL, $has_next_page);
            $params['SCRIPT_VERSION'] = $GLOBALS['sistem_config']->SCRIPT_VERSION;
    ...
    public function transacoes() {
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            $_SESSION['affiliate_logged_datas'] = $this->affiliate_model->load_afiliate_information($_SESSION['logged_id']);
            $_SESSION['affiliate_logged_transactions'] = $this->affiliate_model->load_transactions(
                    $_SESSION['affiliate_logged_datas']['code'], $datas['num_page'] - 1, $GLOBALS['sistem_config']->TRANSACTIONS_BY_PAGE, $datas['token'], $start_date, $end_date, $has_next_page, $status);
            $params['SCRIPT_VERSION'] = $GLOBALS['sistem_config']->SCRIPT_VERSION;
            $params['num_page'] = $datas['num_page'];
                ...
                if ($possible['success']) {
                    ...
                    if ($id_row) {
                        $this->transaction_model->update_transaction_status(
                                $id_row, transactions_status::BEGINNER, $new_beginner_date
                        );
            ...
            if ($_SESSION['front_credit_card'] && $_SESSION['selfie_with_credit_card'] && $_SESSION['open_identity'] && $_SESSION['selfie_with_identity'] && $cpf_upload) {
                ...
                if ($result['success']) {
                    ...
                    if ($uudid_doc) {
                        ...
                        if ($token_signer) {
                            ...
                            if ($result_send) {
                                ...
                                $this->transaction_model->update_transaction_status(
                                        $_SESSION['pk'], transactions_status::WAIT_SIGNATURE);
                            } else {
                                $this->transaction_model->update_transaction_status(
                                        $_SESSION['pk'], transactions_status::PENDING);
                            }
                        } else {
                            $this->transaction_model->update_transaction_status(
                                    $_SESSION['pk'], transactions_status::PENDING);
                    ...
                    $this->transaction_model->save_in_db(
                            'transactions', 'id', $_SESSION['pk'], 'captured', $_SESSION['captured']);
                    ...
                    if ($response['captured'] == 0) {
                        ...
                        $this->transaction_model->save_in_db(
                                'transactions', 'id', $_SESSION['pk'], 'purchase_counter', $purchase_counter);
                        /* ----------------------------------------- */
                        $name = explode(' ', $_SESSION['client_datas']['name']);
                        $name = $name[0];
                        ...
                        if ($payment_method == payment_manager::IUGU) {
                            ...
                            if ($response['LR'] && $response['LR'] != '00') {
                                $report_iugu = $this->iugu_report(
                                        $response['LR'], $_SESSION['transaction_values']['total_cust_value'], $_SESSION['transaction_values']['amount_months']
                                );
    ...
    public function message() {
        ...
        $datas = $this->input->post();
        $result = $this->gmail->send_client_contact_form($datas['name'], $datas['email'], $datas['phone'], $datas['message']);
        if ($result['success'])
            ...
            if (!$this->validate_bank_datas($datas)) {
                ...
                if ($id_row) {
                    ...
                    $result = $this->Gmail->send_mail(
                            $GLOBALS['sistem_config']->ATENDENT_EMAIL, 'Nova solicitação de afiliado', 'Nova solicitação de afiliado', 'O afiliado ' . ['affiliate_datas']['email'] . ' tá pedindo autorizo de cadastro.'
                    );
                    $result['success'] = true;
                } else {
                    $result['success'] = false;
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            do {
                ...
                $transactions = $this->affiliate_model->load_transactions(
                        NULL, $page - 1, $GLOBALS['sistem_config']->TRANSACTIONS_BY_PAGE, $token, $start_date, $end_date, $has_next_page, $status
                );
...
//                    $tr_reduce['credit_card_final'] = $tr['credit_card_final'];
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            do {
                ...
                $transactions = $this->affiliate_model->load_transaction_cutdate(
                        $page - 1, $GLOBALS['sistem_config']->TRANSACTIONS_BY_PAGE, $has_next_page, $init_date, $cut_date
                );
                $page++; //descargar todas las páginas
                foreach ($transactions as $tr) {
                    $id = $tr['id'];
    ...
    public function clean_begginer_images() {
        ...
        do {
            ...
            //guardada en la variable de seccion
            $transactions = $this->affiliate_model->load_transaction_cutdate(
                    $page - 1, $GLOBALS['sistem_config']->TRANSACTIONS_BY_PAGE, $has_next_page, $init_date, $cut_date, transactions_status::BEGINNER
            );
    ...
    public function file_transactions() {
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            while ($result_sql) {
                $result_sql = $this->campaing_model->get_leads_limit($this->session->userdata('id'), $id_campaing, $profile_row['id'], $init_date, $end_date, $info_to_get, $max_id
                );
    ...
    public function approve_transaction() {
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            if ($resp['success']) {
                $this->transaction_model->save_in_db(
                        'transactions', 'id', $_SESSION['transaction_requested_id'], 'ccb_number', $resp['ccb']);
                $this->transaction_model->update_transaction_status(
                        $_SESSION['transaction_requested_id'], transactions_status::TOPAZIO_IN_ANALISYS, true, $resp['ccb']);
                //registrar accion
                $this->load->model('class/watchdog');
                $this->load->model('class/watchdog_type');
                ...
                require_once ($_SERVER['DOCUMENT_ROOT'] . "/lending/src/application/libraries/Gmail.php");
                $this->Gmail = new Gmail();
                $name = explode(' ', $_SESSION['transaction_requested_datas']['name']);
                $name = $name[0];
    ...
    public function request_new_photos() {
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            $this->transaction_model->save_in_db(
                    'transactions', 'id', $_SESSION['transaction_requested_id'], 'new_photos_code', $unique_new_photos_code);
            $this->transaction_model->update_transaction_status(
                    $_SESSION['transaction_requested_id'], transactions_status::WAIT_PHOTO);
    ...
    public function send_new_photos() {
        ...
        if ($transaction) {
            if ($datas['upc'] == $transaction['new_photos_code']) {
                $this->transaction_model->save_in_db(
                        'transactions', 'id', $transaction['client_id'], 'new_photos_code', $transaction['new_photos_code'] . '--used');
    ...
    public function request_new_account() {
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            $this->transaction_model->save_in_db(
                    'transactions', 'id', $_SESSION['transaction_requested_id'], 'new_account_bank_code', $unique_new_account_bank_code);
            $this->transaction_model->update_transaction_status(
                    $_SESSION['transaction_requested_id'], transactions_status::WAIT_ACCOUNT);
    ...
    public function send_new_account() {
        ...
        if ($transaction) {
            if ($datas['uabc'] == $transaction['new_account_bank_code']) {
                $this->transaction_model->save_in_db(
                        'transactions', 'id', $transaction['client_id'], 'new_account_bank_code', $transaction['new_account_bank_code'] . '--used');
    ...
    public function request_new_sing_us() {
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            $this->transaction_model->save_in_db(
                    'transactions', 'id', $_SESSION['transaction_requested_id'], 'new_sing_us_code', $unique_new_sing_us_code);
            ...
            if ($uudid_doc) {
                ...
                if ($token_signer) {
                    ...
                    if ($result_send) {
                        //4. cambiar el status de la transaccion
                        $this->transaction_model->update_transaction_status(
                                $_SESSION['transaction_requested_id'], transactions_status::WAIT_SIGNATURE);
    ...
    public function request_recuse_and_reverse_money() {
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            if ($resp['success']) {
                ...
                $this->transaction_model->update_transaction_status(
                        $_SESSION['transaction_requested_id'], transactions_status::REVERSE_MONEY);
    ...
    public function update_transaction_datas_by_id() {
        ...
        if ($_SESSION['logged_role'] === 'ADMIN') {
            ...
            if (isset($datas['edit_trans_name']))
                $personal_datas['name'] = $datas['edit_trans_name'];
            if (isset($datas['edit_trans_email']))
                $personal_datas['email'] = $datas['edit_trans_email'];
            if (isset($datas['edit_trans_phone_ddd']))
                $personal_datas['phone_ddd'] = $datas['edit_trans_phone_ddd'];
            if (isset($datas['edit_trans_phone_number']))
                $personal_datas['phone_number'] = $datas['edit_trans_phone_number'];
            if (isset($datas['edit_trans_cep']))
                $personal_datas['cep'] = $datas['edit_trans_cep'];
            if (isset($datas['edit_trans_street_address']))
                $personal_datas['street_address'] = $datas['edit_trans_street_address'];
            if (isset($datas['edit_trans_number_address']))
                $personal_datas['number_address'] = $datas['edit_trans_number_address'];
            if (isset($datas['edit_trans_complement_address']))
                $personal_datas['complement_number_address'] = $datas['edit_trans_complement_address'];
            if (isset($datas['edit_trans_city_address']))
                $personal_datas['city_address'] = $datas['edit_trans_city_address'];
            if (isset($datas['edit_trans_state_address']))
                $personal_datas['state_address'] = $datas['edit_trans_state_address'];
            $a = $this->transaction_model->update_db_steep_1($personal_datas, $id);

            if (isset($datas['edit_trans_credit_card_name']))
                $credit_card_datas['credit_card_name'] = $datas['edit_trans_credit_card_name'];
            $b = $this->transaction_model->update_db_steep_2($credit_card_datas, $id);

            if (isset($datas['edit_trans_bank_code']))
                $bank_datas['bank'] = $datas['edit_trans_bank_code'];
            if (isset($datas['edit_trans_agency']))
                $bank_datas['agency'] = $datas['edit_trans_agency'];
            if (isset($datas['edit_trans_account']))
                $bank_datas['account'] = $datas['edit_trans_account'];
            if (isset($datas['edit_trans_dig']))
                $bank_datas['dig'] = $datas['edit_trans_dig'];
            if (isset($datas['edit_account_type']))
                $bank_datas['account_type'] = $datas['edit_account_type'];
        ...
        if ($_SESSION['is_possible_steep_1'] && $_SESSION['is_possible_steep_2'] && $_SESSION['is_possible_steep_3'] && $datas['key'] === $_SESSION['key']) {
            ...
            if (is_dir($path_name) || mkdir($path_name, 0755)) {
                ...
                if ($fileError == UPLOAD_ERR_OK) {
                    ...
                    if ((($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "application/pdf") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/pjpeg") || ($_FILES["file"]["type"] == "image/x-png") || ($_FILES["file"]["type"] == "image/png")) && ($_FILES["file"]["size"] < 10485761) && in_array($extension, $allowedExts)) {
                        if ($_FILES["file"]["error"] > 0) {
                            ...
                            move_uploaded_file($_FILES["file"]["tmp_name"], $path_name . "/" . $filename);
        ...
        if ($_SESSION['session_new_foto']) {
            ...
            if (is_dir($path_name) && count($client) == 1) {
                ...
                if ($fileError == UPLOAD_ERR_OK) {
                    ...
                    $temp = explode(".", $_FILES["file"]["name"]);
                    $extension = strtolower(end($temp));
                    if ((($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/pjpeg") || ($_FILES["file"]["type"] == "image/x-png") || ($_FILES["file"]["type"] == "image/png")) && ($_FILES["file"]["size"] < 10485761) && in_array($extension, $allowedExts)) {
                        if ($_FILES["file"]["error"] > 0) {
                            $result['message'] .= "Return Code: " . $_FILES["file"]["error"];
                        } else {
                            $file_names = ["front_credit_card", "selfie_with_credit_card", "open_identity", "selfie_with_identity", "cpf_card"];
    ...
    public function upload_document_D4Sign($id) {
        ...
        try {
            ...
            if (is_object($id_doc) && $id_doc->message == "success")
                $this->transaction_model->save_in_db(
                        'transactions', 'id', $id, 'doc_d4sign', $id_doc->uuid);
        } catch (Exception $e) {
            //echo $e->getMessage();
            return null;
        }
    ...
    public function signer_for_doc_D4Sign($id, $act = '1', $foreign = 0, $certificadoicpbr = 0, $assinatura_presencial = 0, $embed_methodauth = 'email') {
        ...
        try {
            ...
            if (is_object($result) && $result->message[0]->success) {
                $this->transaction_model->save_in_db(
                        'transactions', 'id', $id, 'key_signer', $result->message[0]->key_signer);
    ...
    public function upload_document_template_D4Sign($id) {
        ...
        $this->transaction_model->save_in_db(
                'transactions', 'id', $id, 'contract_id', $document_id);
        ...
        try {
            ...
            if (is_object($return) && $return->uuid != "") {
                $this->transaction_model->save_in_db(
                        'transactions', 'id', $id, 'doc_d4sign', $return->uuid);
                    ...
                    if ($livre_tr['status_id'] == transactions_status::TOPAZIO_IN_ANALISYS) {
                        switch ($transaction->statusCode) {
                            ...
                            case 2100: //TOPAZIO - "CANCELADA"
                                ...
                                $this->load->model('class/transaction_model');
                                $this->transaction_model->update_transaction_status(
                                        $livre_tr['client_id'], transactions_status::PENDING);
                    ...
                    if ($livre_tr['status_id'] == transactions_status::TOPAZIO_IN_ANALISYS) {
                        switch ($transaction->statusCode) {
                            ...
                            case 2500: //TOPAZIO - "PAGA CONFIRMADA"
                                ...
                                $this->load->model('class/transaction_model');
                                $this->transaction_model->update_transaction_status(
                                        $livre_tr['client_id'], transactions_status::TOPAZIO_APROVED);
        ...
        do {
            ...
            foreach ($transactions as $transaction) {
                ...
                if ($signature_status[0]->statusId == 4) {
                    //documento assinado                
                    $this->transaction_model->update_transaction_status(
                            $transaction['id'], transactions_status::PENDING);
